...
//		System.out.println("clientId"+clientId+" client Secret"+clientSecret);
	...
	public List<String> deleteLanguageSkillsFromCSV(MultipartFile file, String organizationName
		...
		if(clientId==null || clientSecret==null || credentials.isEmpty()) {
			results.add("Error: Client credentials not found for organization"+organizationName);
			return results;
		}
		...
		try {
			...
			for(String languageSkillName: languageSkillNames) {
				...
				try {
					...
					List<Language> languageSkills = routingApi.getRoutingLanguages(pageSize,pageNumber,"ASC",languageSkillName,null).getEntities();
		...
		try(InputStream inputStream = file.getInputStream();
			...
			if(scanner.hasNextLine()) {
				...
				if(headers.length==0 || !headers[0].equalsIgnoreCase("Delete LanguageSkill")) {
					throw new IllegalArgumentException("Invalid file format: the first cell of first row must be 'Delete LanguageSkill'");

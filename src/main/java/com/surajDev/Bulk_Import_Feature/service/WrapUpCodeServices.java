...
public class WrapUpCodeServices {
	...
	public List<String> createWrapUpCodeFromCSV(MultipartFile file,String organizationName, String environment)throws IOException{
		...
		//Parse the csv to extract wrap up code and division Names
			...
			for(String[] row: csvData) {
				...
				try {
					...
					results.add("Successfully Created wrap-up Code: "+ wrapUpCodeName +" in division: '"+divisionName+"'" );
				}
				catch(ApiException e) {
					results.add("Failed to create wrap-up code: "+ wrapUpCodeName +" (already exist or other "+e.getMessage()+" )");
	...
	public List<String> deleteWrapUpCodeFromCSV(MultipartFile file, String organizationName, String environment)throws IOException{
		List<String> results = new ArrayList<>();
			...
			RoutingApi routingApi = new RoutingApi(apiClient);
			 ...
			 for(String[] row: csvData) {
				 ...
				 try {
					 ...
					 List<WrapupCode> wrapUpCodes = routingApi.getRoutingWrapupcodes(100,1,null,null,wrapUpCodeName,null, divisionId).getEntities();
					 ...
					 for(WrapupCode wrapUpCode: wrapUpCodes) {
						 ...
						 results.add("Successfully deleted wrap-up code: " +wrapUpCodeName +" from Division "+divisionName);
					 }
					 
				 }catch(ApiException e)
				 {
					 results.add("Failed to delete the wrap Up code "+ wrapUpCodeName+ " (Error: "+ e.getMessage()+")");
				 }
			 }
	...
	private AuthzDivision fetchDivisionByName(String divisionName, ObjectsApi apiInstance) throws IOException, ApiException {
		
		
		AuthzDivisionEntityListing  divisions = apiInstance.getAuthorizationDivisions(100,1,null,null,null,null,null,null,divisionName);
	...
	private List<String[]> parseCSV(MultipartFile file, List<String>results)throws IOException{
		...
		try(InputStream inputStream = file.getInputStream();
			...
			if(scanner.hasNextLine()) {
				...
				//validate the header
				if(headers.length<2 || !headers[0].equalsIgnoreCase("WrapUpCode Name") || !headers[1].equalsIgnoreCase("Division Name")) {
					throw new IllegalArgumentException("Invalid File Format: The first cell of first row must be 'WrapUpCode Name'and first cell of second column must be 'Division Name'");
			...
			while(scanner.hasNextLine()) {
				...
				if(columns.length>=2 && !columns[0].trim().isEmpty() &&!columns[1].trim().isEmpty()) {
					...
					if(!wrapUpCodeName.matches(wrapUpCodeNamePattern) && !divisionName.matches(wrapUpCodeNamePattern)) {
						results.add("Error: Invalid wrapup code or division name '"+wrapUpCodeName+"' or ' "+divisionName+"'at row "+rowNumber+". Only letters, digits, underscore, and space are allowed.");
						continue;
	...
	private List<String[]> deleteParseCSV(MultipartFile file, List<String> results)throws IOException{
		...
		try(InputStream inputStream = file.getInputStream();
			...
			if(scanner.hasNextLine()) {
				...
				//validate the header
				if(headers.length<2 || !headers[0].equalsIgnoreCase("DeleteWrapUpCode Name") || !headers[1].equalsIgnoreCase("Division Name")) {
					throw new IllegalArgumentException("Invalid File Format: The first cell of first row must be 'DeleteWrapUpCode Name'and first cell of second column must be 'Division Name'");
